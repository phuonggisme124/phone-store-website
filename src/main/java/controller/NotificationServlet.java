...
public class NotificationServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Customer user = (Customer) (session != null ? session.getAttribute("user") : null);
        ...
        try (Connection conn = new DBContext().conn) {
            ...
            List<Notification> notifications = dao.getNotificationsByUser(user.getCustomerID());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = new DBContext().conn) {
            NotificationDAO dao = new NotificationDAO(conn);
            dao.markAllAsRead(user.getCustomerID()); // đánh dấu tất cả chưa đọc thành đã đọc
