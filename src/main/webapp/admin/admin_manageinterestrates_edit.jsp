...
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>Admin Dashboard - Update Interest Rate</title>
    ...
    <body>
        <div class="d-flex" id="wrapper">
            ...
            <div class="page-content flex-grow-1">
                ...
                <% InterestRate interestRate = (InterestRate) request.getAttribute("interestRate");%>
                ...
                <div div class="container-fluid p-4">
                    <form action="interestrates" method="post">
                        <div class="form-header text-center">
                            <h2 class="fw-bold text-primary mb-1">Update Interest Rate</h2>
                            <p class="text-muted">Edit information</p>
                        </div>
                        <h5 class="text-secondary border-bottom pb-2 mb-3"><i class="bi bi-person-vcard me-2"></i>Contact Info</h5>
                        <div class="mb-3">
                            <input type="hidden" >
                            <input type="hidden" id="id" name="id" class="form-control" 
                                   value="<%= interestRate.getInterestRateID()%>" readonly>
                        </div>

                        <div class="mb-3">
                            <label for="period">Period</label>
                            <input type="number" id="period" name="period" class="form-control" 
                                   value="<%= interestRate.getInstalmentPeriod()%>" required placeholder="Ví dụ: 6">
                        </div>

                        <div class="mb-3">
                            <label for="rateValue">Interest Rate</label>
                            <input type="number" step="0.01" id="rateValue" name="rateValue" class="form-control" 
                                   value="<%= interestRate.getPercent()%>" required placeholder="Ví dụ: 5.5">
                        </div>

                        <div class="mb-3">
                            <label for="rateValue">Penalty (Expired)</label>
                            <input type="number" step="0.01" id="rateExpired" name="rateExpired" class="form-control" 
                                   value="<%= interestRate.getPercentExpried()%>" required placeholder="Ví dụ: 5.5">
                        </div>

                        <div class="btn-group">
                            <a href="interestrates?action=viewInterestRate" class="btn btn-cancel">Cancel</a>
                            

                            <button type="submit" name="action" value="deleteInterestRate" class="btn btn-save"> Delete</button>
                            <button type="submit" name="action" value="updateInterestRate" class="btn btn-save">💾 Update</button>
                        </div>
                    </form>
                </div>
            </div>
