    ...
    <body class="bg-gray-100">
        ...
        <div id="paymentMethodModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center p-0 sm:p-4 z-[60] hidden">
            <div class="bg-white rounded-t-lg sm:rounded-lg shadow-xl w-full max-w-2xl">
                ...
                <div class="p-4 space-y-3 max-h-[70vh] overflow-y-auto">
                    ...
                    <div id="openTransferModalBtn" class="payment-option border rounded-lg p-3 flex items-center cursor-pointer" data-value="TRANSFER" data-text="Thanh toán bằng chuyển khoản" data-icon="https://cellphones.com.vn/smember/_nuxt/img/vnpay.c0bd59a.png">
                        <img src="https://cellphones.com.vn/smember/_nuxt/img/vnpay.c0bd59a.png" class="w-10 h-10 mr-4 object-contain">
                        <span class="font-medium">Thanh toán bằng chuyển khoản</span>
                    </div>
                </div>
                        ...
                        <div class="flex text-center text-sm">
                            ...
                            <div class="w-3/4 flex">
                                ...
                                <div class="term-column w-1/3 border rounded-md p-2 mx-1 flex flex-col" data-term="<%=iR.getInstalmentPeriod()%>">
                                    <div class="flex-grow">
                                        <div class="h-12 flex items-center justify-center font-bold term-header"><%=iR.getInstalmentPeriod()%> tháng</div>
                                        <div class="h-10 flex items-center justify-center"><%=currencyFormat.format(instalmentPrice)%></div>
                                        <div class="h-10 flex items-center justify-center text-blue-600 font-semibold"><%=iR.getPercent()%>%/năm</div>
                                        <div class="h-10 flex items-center justify-center font-semibold"><%=currencyFormat.format(totalPriceEachMothPay)%></div>
                                        <div class="h-10 flex items-center justify-center font-semibold"><%=currencyFormat.format(totalPriceAfterInstalment)%></div>
                                    </div>
        ...
        <div id="transferModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center p-0 sm:p-4 z-[70] hidden">
            <div class="bg-white rounded-t-lg sm:rounded-lg shadow-xl w-full max-w-md">
                <div class="p-4 border-b flex items-center relative">
                    ...
                    <h2 class="text-lg font-semibold text-gray-800 w-full text-center">Thông tin chuyển khoản</h2>
                    <button class="js-close-modal absolute right-4 text-gray-400 hover:text-gray-600"><i class="fa-solid fa-times fa-lg"></i></button>
                </div>
                <div class="p-6 text-center">
                    <p class="text-sm text-gray-600 mb-4">Quét mã QR để thanh toán hoặc chuyển khoản thủ công theo thông tin dưới đây.</p>
                    ...
                    <div class="space-y-3 text-left bg-gray-50 p-4 rounded-lg">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-bold text-lg text-red-600"><%= currencyFormat.format(totalPriceAfterDiscount)%></span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-gray-500">Ngân hàng:</span>
                            <span class="font-semibold text-gray-800">MB Bank (Ngân hàng Quân đội)</span>
                        </div>
                        <div class="flex justify-between items-center">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <button id="copyContentBtn" class="text-gray-500 hover:text-blue-600" title="Sao chép">
                                    <i class="fa-regular fa-copy"></i>
                                </button>
                            </div>
                        </div>
                    </div>
                    <p class="text-xs text-gray-500 mt-4">Lưu ý: Vui lòng nhập đúng nội dung chuyển khoản để đơn hàng được xác nhận tự động.</p>
                </div>
                <div class="p-4 border-t">
                    <button id="confirmTransferBtn" class="w-full bg-red-600 text-white font-bold py-3 rounded-lg hover:bg-red-700">
                        Xác nhận
                    </button>
                </div>
            </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                // FIX: Gắn lại logic tạo mã QR động khi mở modal chuyển khoản
                ...
                openTransferModalBtn.addEventListener('click', (e) => {
                    ...
                    const addInfo = encodeURIComponent('TT ' + orderId);
                    const encodedAccountName = encodeURIComponent(accountName);

                    // --- SỬA LỖI Ở ĐÂY ---
                    // Thêm một tham số timestamp để trình duyệt không dùng lại ảnh cũ bị lỗi
                    const cacheBuster = `&t=${Date.now()}`;
                    const vietQrApiUrl = `https://img.vietqr.io/image/${bankId}-${accountNumber}-compact.png?amount=${totalAmount}&addInfo=${addInfo}&accountName=${encodedAccountName}${cacheBuster}`;
                    // --- KẾT THÚC SỬA LỖI ---

                    console.log("URL được tạo ra (đã thêm cache buster):", vietQrApiUrl);
                ...
                // Chọn phương thức thanh toán đơn giản (COD)
                ...
                // FIX: Cải thiện logic nút xác nhận PTTT
                confirmPaymentBtn.addEventListener('click', () => {
                    ...
                    if (selectedOption && selectedOption.dataset.value === 'COD') {
                        ...
                        alert('Vui lòng nhấn vào phương thức đã chọn để tiếp tục.');
                    } else {
                ...
                confirmInstallmentBtn.addEventListener('click', () => {
                    if (selectedInstallmentTerm) {
                        ...
                        const icon = document.getElementById('openInstallmentModalBtn').dataset.icon;
